...
def test_prompt_selects_ai_examples_from_area_filter():
    prompt = build_search_strategy_prompt(["ai.nlp"])
...
def test_prompt_routes_specialized_ai_areas_to_distinct_examples():
    safety_prompt = build_search_strategy_prompt(["ai.safety"])
    reasoning_prompt = build_search_strategy_prompt(["ai.reasoning"])
...
def test_prompt_can_combine_two_area_example_sets_without_unrelated_examples():
    prompt = build_search_strategy_prompt(["med.general", "econ.general"])
